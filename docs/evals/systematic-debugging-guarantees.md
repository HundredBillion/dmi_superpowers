...
whole file. Every hit traces back to G1-G6 above, or to one of two
...
(lines 273-285) and "Your human partner's signals" list (lines 262-271),
which are illustrative catalogs restating G1/G2/G4/G6 rather than adding new
enforceable gates. No guarantee was found beyond the 6 above. (This skill
enumerates 6, one more than the sibling `writing-skills` analysis — the
brief's seed list of 5 omitted the Red-Flags STOP table as its own
guarantee, but the task instructions explicitly call it out as core, and it
reads as a distinct, self-contained gate rather than a restatement, so it is
promoted to G6 here.)
...
| 6-12 | Title, intro paragraph ("skip phases only when you can explicitly justify it"), CONTEXT.md/ADR orientation note, "violating the letter is violating the spirit" | RESIDENT | frames G1-G6 |
...
| 65-80 | "Locating *where* it breaks" sub-technique intro + pseudocode | DEFERRABLE | — (sub-procedure) |
...
| 182-186 | Tool preference (debugger > targeted logs > never "log everything") | DEFERRABLE | — (recommendation; the "never log everything" red flag is independently resident in G6's table) |
...
| 239-241 | "Guardrails (always-on)" intro sentence | RESIDENT | frames G6 |
...
| 262-271 | "Your human partner's signals you're doing it wrong" — phrase catalog | DEFERRABLE | — (illustrative catalog restating G6's STOP trigger, keyed to human phrasing rather than a new gate) |
| 273-285 | Common Rationalizations table | DEFERRABLE | — (explicit call-out in the task brief; restates G1/G2/G4/G5) |
| 287-296 | "When the process reveals 'no root cause'" edge-case procedure | DEFERRABLE | — (scenario-specific guidance, not one of the 6 core gates) |
...
blocks not attributed to either side): **≈ 8,873 chars**.

`projected_deferrable_savings ≈ 9,656 / 4 ≈ 2,414 est. tokens`

Trimmed body would be ≈ 8,873 chars ≈ 2,218 tokens (before any glue text
...
**GO** — projected savings (~2,414 tokens) clears the ~1,000-token floor,
though by a much narrower margin than the `writing-skills` sibling analysis
(~5,561 tokens on a file with far more deferrable prose). The RESIDENT set
here is large relative to file size (~48% of chars) because this skill is a
gate-sequence: nearly every phase's entry/exit condition is independently
load-bearing. The deferrable set is real but concentrated in a few
extractable units: the loop-construction menu (50-63), the multi-layer
instrumentation example (65-100), Pattern Analysis (161-170), the
rationalizations table (273-285), and the supporting-files/related-skills
list (298-311) — together these account for the bulk of the ~9,656
deferrable chars. A subsequent trim pass (Task 7, if pursued) should target
those five spans rather than attempt broad cuts across the phase headers.
